	...
	public class BallCreator : MonoBehaviour
	{
		...
		public void CreateClone()
		{
			foreach (var item in _saveBalls)
			{
				if (item != null)
				{
					GameObject cloneOne = Instantiate(ballPrefab, new Vector3(item.transform.position.x, item.transform.position.y), Quaternion.identity, null);
					GameObject cloneTwo = Instantiate(ballPrefab, new Vector3(item.transform.position.x, item.transform.position.y), Quaternion.identity, null);
